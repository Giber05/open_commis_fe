...
export class CreateComPost {
  private manageComPostRepo: ManageComPostRepo = new ManageComPostRepoImpl();

  async execute(params: { token: string; formData: any }): Promise<Resource<ComPostDetailModel>> {
    return await this.manageComPostRepo.createComPost({
      token: params.token,
