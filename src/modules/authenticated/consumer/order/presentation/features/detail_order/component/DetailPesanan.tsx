import { Card, Divider, Rate, Row, Typography, Image, Col, Tag } from 'antd';
import React from 'react'

function DetailPesanan() {
  return (
    <Card
      className="comic-shadow sm:shrink-0"
    //  loading={isLoadingComPosts}
      style={{
        minHeight: "360px",
        maxWidth:"580px",
        padding: "0",
      }}
    >
       <Typography className='text-center text-lg sm:text-lg lg:text-lg font-bold'>Detail Pesanan</Typography>
      <Divider className="my-2" />
       <Row justify='space-between'>
            <Col span={13} className="text-base leading-10">
                <Col>
                    <Typography.Text className="text-sm mb-1 sm:text-sm lg:text-lg pr-5">Status Pesanan</Typography.Text>
                </Col>
                <Col>
                    <Typography.Text className="text-sm mb-1 sm:text-sm lg:text-lg ">Perkiraan waktu selesai</Typography.Text>
                </Col>
                <Col>
                    <Typography.Text className="text-sm mb-1 sm:text-sm lg:text-lg ">Tanggal Pemesanan</Typography.Text>
                </Col>
            </Col>
            <Col span={11} className="text-base leading-10" >
                <Col>
                    <Tag color="blue" className="text-xs mb-1 sm:text-xs lg:text-lg text-right">Menunggu Konfirmasi</Tag>
                </Col>
                <Col>
                    <Typography.Text className="text-sm mb-1 sm:text-sm lg:text-lg font-bold text-right"> 3 Hari</Typography.Text>
                </Col>
                <Col>
                    <Typography.Text className="text-sm mb-1 sm:text-sm lg:text-lg font-bold text-right">22 Desember 2022</Typography.Text>
                </Col>
            </Col>
      </Row>
        <div className="text-center">
            <Typography>Deskripsi Permintaan</Typography>
        </div>
        <div className="text-justify">
            <Typography>But I must explain to you how all this mistaken idea 
                of denouncing pleasure and praising pain was born and I will 
                give you a complete account of the system, and expound the actual 
                teachings of the great explorer of the truth </Typography>
        </div> 
        <div className="text-center">
        <Image
          src={'asd'}
          preview={false}
          loading="eager"
          className="max-h-44 object-contain"
          fallback="data:image/png;base64,"
        />
      </div> 
    </Card>
  );
}

export default DetailPesanan